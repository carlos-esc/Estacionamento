...
public class EntradaValor extends PlainDocument {
    ...
    public EntradaValor(int maxLen, int qtdDec, char mil, char dec) {
        super();
        iMaxLength = 0;
        if (maxLen > 0) {
            iMaxLength = maxLen;
    ...
    public void insertString(int offs, String str, AttributeSet attrSet) throws BadLocationException {
        ...
        while ((texto.length() > 0) && ((texto.charAt(0) == '0') || (texto.charAt(0) == sDec.charAt(0)))) {
            texto = texto.substring(1);
        }
        // verificar se o que esta sendo digitado ou colado é um valor válido    
        for (int i = 0; i < str.length(); i++) {
            char c = str.charAt(i);
            if ((!Character.isDigit(c)) && (!sMil.equals(Character.toString(c))) && (!(sDec.equals(Character.toString(c)) && (iQtdDec > 0)))) {
                // o sinal de negativo deve ser sempre o primeiro a ser informado
                if (!((texto.length() == 0) && (i == 0) && ("-".equals(Character.toString(c))))) {
                    return;
                }
            }
        }
        ...
        if ((texto.length() < this.iMaxLength)
            ...
            if (((s.length() + (iQtdDec == 0 ? 0 : 1)) > this.iMaxLength) && (this.iMaxLength != 0)) {
                return;
            }
